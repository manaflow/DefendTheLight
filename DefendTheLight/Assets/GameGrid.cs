...
public class GameGrid : MonoBehaviour
{
    ...
    public List<int> GetPath(int i, int j)
    {
        ...
        while(minQue.Count > 0)
        {
            Node node = minQue[0];
            minQue.RemoveAt(0);
            Node up = Search(node.row - 1, node.col, node.cost + 1, ref node.path, 1, ref objectGrid, ref pathGrid);
            Node right = Search(node.row, node.col + 1, node.cost + 1, ref node.path, 2, ref objectGrid, ref pathGrid);
            Node down = Search(node.row + 1, node.col, node.cost + 1, ref node.path, 3, ref objectGrid, ref pathGrid);
            Node left = Search(node.row, node.col - 1, node.cost + 1, ref node.path, 4, ref objectGrid, ref pathGrid);
